...
See [examples/standard_v2_instance](examples/standard_v2_instance) for a full working example.
...
Post-deploy tests in `tests/post_deploy_functional/` and `tests/post_deploy_functional_readonly/` target `examples/standard_v2_instance` via an explicit folder constant in each `main_test.go`. Adding another example requires a new test entry point or updating that constant; it is not picked up automatically.
...
| <a name="input_appgw_private_ip"></a> [appgw\_private\_ip](#input\_appgw\_private\_ip) | Static Private IP for Application Gateway. Must be within the range of the subnet used for Application gateway. Mandatory when<br/>    variable `appgw_private` is set to `true`. | `string` | `null` | no |
...
| <a name="input_subnet_id"></a> [subnet\_id](#input\_subnet\_id) | Subnet ID for attaching the Application Gateway. This is a mandatory field. All App Gateway must be assigned a subnet. | `string` | n/a | yes |
